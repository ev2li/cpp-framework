...
bool Socket::getOption(int level, int option, void* result, size_t* len){
    ...
    if(rt){
        SYLAR_LOG_DEBUG(g_logger) << "getOpti sock = "  << m_sock
...
bool Socket::setOption(int level, int option, const void* result, size_t len){
    if(auto rt = setsockopt(m_sock, level, option, result, (socklen_t)len)){
        SYLAR_LOG_DEBUG(g_logger) << "getOpti sock = "  << m_sock
                 << " level="  << level  << " option=" << option
                 << " errno=" << errno << " errstr=" << strerror(errno);
        return false;
...
Socket::ptr Socket::accept(){
    ...
    if(newsock == -1){
        SYLAR_LOG_INFO(g_logger) << "accept(" << m_sock << ") errno="
...
bool Socket::bind(const Address::ptr addr){
    ...
    if(addr->getFamily() != m_family){
        SYLAR_LOG_INFO(g_logger) << "bind sock.family("
    ...
    if(::bind(m_sock, addr->getAddr(), addr->getAddrLen())){
        SYLAR_LOG_INFO(g_logger) << "bind errno=" << errno
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms = -1){
    ...
    if(SYLAR_UNLIKELY(addr->getFamily() != m_family)){
        SYLAR_LOG_INFO(g_logger) << "connect sock.family("
             << m_family << ") addr.family("  << addr->getFamily()
              << ") not equal, addr=" << addr->toString();
        return false;
    }
    ...
    if(timeout_ms == (uint64_t)-1){
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())){
            SYLAR_LOG_INFO(g_logger) << "sock="  << m_sock  << " connect(" << addr->toString()
...
bool Socket::listen(int backlog = SOMAXCONN){
    if(!isValid()){
        SYLAR_LOG_INFO(g_logger) << "listen error sock = -1";
        return false;
    }
    if(::listen(m_sock, backlog)){
        SYLAR_LOG_INFO(g_logger) << "listen error errno=" << errno
